...
export function mergeAndScoreFindings(findings: Finding[]): Finding[] {
  const byKey = new Map<string, Finding & { sources: Set<string> }>();
  ...
  for (const f of findings) {
    ...
    if (!existing) {
      ...
      // keep longer message
      if ((f.message?.length || 0) > (existing.message?.length || 0)) existing.message = f.message;
