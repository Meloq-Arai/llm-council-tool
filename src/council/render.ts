import type { FinalOutput, FinalIssue } from './schema.js';
...
function tokensLine(usage?: Record<string, any>): string {
  if (!usage) return '';
  let totalIn = 0;
  let totalOut = 0;
  let total = 0;
...
function renderIssue(i: FinalIssue): string {
...
export function renderMarkdown(out: FinalOutput): string {
