import { defaultStages, StageDef } from './stages.js';
...
export type CouncilRunOptions = {
  ...
  maxCostUsd?: number; // placeholder for later
};

export async function runCouncil(opts: CouncilRunOptions): Promise<CouncilResult> {
  ...
  if (synthesizer) {
    ...
    const synthPrompt =
      `You are the Synthesizer. Merge the council findings into a final review.\n` +
...
function renderMarkdownSummary(findings: ReturnType<typeof mergeAndScoreFindings>, repoLabel?: string): string {
  const header = repoLabel ? `# LLM Council Review — ${repoLabel}\n\n` : `# LLM Council Review\n\n`;
  if (!findings.length) return header + 'No findings.\n';
