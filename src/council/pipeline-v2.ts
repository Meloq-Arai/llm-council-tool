...
function normalizeIssue(x: any, idx: number): FinalIssue {
  ...
  const inferLineRange = () => {
    if (rNums.length) {
      ...
      return { start: min, end: max, side: 'RIGHT' as const };
    }
    if (lNums.length) {
      const min = Math.min(...lNums);
      const max = Math.max(...lNums);
      return { start: min, end: max, side: 'LEFT' as const };
    }
    return undefined;
  };
...
const isTokenLimit = (e: any) =>
  /tokens_limit_reached/i.test(String(e?.message ?? e ?? '')) ||
...
export async function runCouncil(cfg: CouncilConfig): Promise<{ final: FinalOutput; markdown: string; usage: any }> {
  ...
  const verifierSystem =
    `You are a strict confidence checker. For each issue, verify it against the diff.\n` +
